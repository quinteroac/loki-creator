...
class HtmlCardFactory:
    def create(self, *, prompt: str, source_tool_id: str | None = None, name: str = "Canvas card") -> GeneratedCard:
  ...
  <div style="width:68%;min-height:52%;display:grid;place-items:center;border-radius:28px;background:linear-gradient(135deg,#245cff,#e9429f);text-align:center;padding:24px;">
    <div>
      <strong style="display:block;font-size:20px;line-height:1.4;">Generated Canvas</strong>
      <span style="display:block;margin-top:8px;color:rgba(255,255,255,.72);font-size:13px;line-height:1.5;">{escaped_prompt[:72]}</span>
    </div>
  </div>
</section>"""
