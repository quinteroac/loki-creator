...
def write_data_url_media(data_url: str, destination: Path, kind: str) -> Path | None:
    mime_type, data = parse_data_url(data_url)
    if not mime_type.startswith(f"{kind}/"):
        return None
    path = destination.with_suffix(extension_for_mime_type(mime_type, kind))
...
def resolve_media_source(src: str, destination: Path, kind: str) -> tuple[Path, str] | None:
    ...
    if src.startswith(f"data:{kind}/"):
        path = write_data_url_media(src, destination, kind)
        if path is not None:
